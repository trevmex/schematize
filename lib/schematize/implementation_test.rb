...
module Schematize
  class ImplementationTest
    def initialize(schema, options={:output_dir => File.absolute_path(File.join(File.dirname(__FILE__), '..', 'output'))})
      @implementation_test = Schematize::Parser.new(schema, {:output_dir => options[:output_dir], :template_file_name => 'ImmutableSchemaTest.java.erb', :language => 'java', :test => true, :impl => true})
    end
